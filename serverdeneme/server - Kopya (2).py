...
def _enrich_orders(orders: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    out = []
    for o in orders:
        out.append({**o, **_order_color_and_days(o.get("datetime", ""))})
    return out
...
async def index(request: Request,
    ...
    # 1) "TÜMÜ" görüntüsünde iptaller gizlenir
    if durum == "TÜMÜ":
    ...
    if platform != "TÜMÜ":
        orders = [o for o in orders if (o.get("entegration") or "") == platform]
    if kargo != "TÜMÜ":
        orders = [o for o in orders if (o.get("cargo_company") or "") == kargo]
...
async def toggle_item(no: str, request: Request):
    # Hem JSON hem form yakala (htmx bazen form gönderir)
    try:
...
async def order_cancel(no: str):
    ...
    save_orders_to_json(orders)
    # detaya dön (htmx 303 ile yenile)
